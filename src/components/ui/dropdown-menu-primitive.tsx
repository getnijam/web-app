...
/* eslint-disable -- Vendored from animate-ui (primitives/radix/dropdown-menu), adapted for our paths/asChild; kept close to upstream rather than reformatted to our lint rules. */
...
// Unlike the rest of the menu the item carries no motion of its own (the sliding
// highlight handles that), so we support `asChild` to let callers render a link.
...
function DropdownMenuItem({
  ...
  return (
    ...
    >
      {asChild ? (
        <Slot.Root data-slot="dropdown-menu-item" data-disabled={disabled} {...props} />
      ) : (
        <div data-slot="dropdown-menu-item" data-disabled={disabled} {...props} />
      )}
    </DropdownMenuPrimitive.Item>
  );
}
