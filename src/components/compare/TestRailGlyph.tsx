/**
 * The TestRail brand mark (the stacked-diamonds logo), inlined as an SVG, a
 * competitor logo, which is the sanctioned exception to the Hugeicons-only rule.
 * Single path with `fill: currentColor`, so it takes the surrounding text color
 * (e.g. `text-testrail`). Decorative, so aria-hidden and always paired with a
 * visible "TestRail" label.
 */
export function TestRailGlyph({ className }: { className?: string }) {
  return (
